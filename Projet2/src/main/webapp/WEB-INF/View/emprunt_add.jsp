	      ...
	      <form action="/TP3Ensta/emprunt_add" method="post" class="col s12">
	        <div class="row">
	          <div class="input-field col s6">
                ...
                <c:forEach var="livre" items="${livresDispo}">
                  <option value="${livre.getId()}"><c:out value="${livre.getTitre()}"/>, de <c:out value="${livre.getAuteur()}"/></option>
                </c:forEach>
	            </select>
	          </div>
	          <div class="input-field col s6">
                ...
                <c:forEach var="membre" items="${membresDispo}">
                  <option value="${membre.getId()}"><c:out value="${membre.getPrenom()}"/> <c:out value="${membre.getNom()}"/></option>
                </c:forEach>
	            </select>
	          </div>
	        </div>
