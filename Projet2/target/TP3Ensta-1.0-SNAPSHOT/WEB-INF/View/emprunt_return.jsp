	          ...
	          <div class="input-field col s12">
	            <select id="id" name="id" class="browser-default">
                  ...
                  <c:forEach var="emprunt" items="${emprunts}">
                    <option value="${emprunt.getId()}" ${(emprunt.getId() == id) ? " selected" : ""}><c:out value="${livresEmprunt.get(i).getTitre()}"/>, emprunté par <c:out value="${membresEmprunt.get(i).getPrenom()}"/> <c:out value="${membresEmprunt.get(i).getNom()}"/></option>
                    <%
                      i++;
                      pageContext.setAttribute("i", new Integer(i));
                    %>
